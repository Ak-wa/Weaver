...
class DirBruter:
    ...
    def urlenum(self, current_dir):  # Interprets HTTP Status Codes & sorts into lists
        try:
            global Zcounter
    ...
    def dirBruter(self):  # Starts threads and prints lists
        with open(wordlist) as dirs:
            try:
                ...
                for onedir in dirs:
                    try:
                        t = threading.Thread(target=self.urlenum, args=(onedir,))
                        t.daemon = True
                        t.start()
                        threads.append(t)
                        bar.next()
